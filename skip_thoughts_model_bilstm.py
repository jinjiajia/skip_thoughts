...
def attention(config, emb, mask, scope):
  for i in range(config.num_blocks):
      with tf.variable_scope("enc_num_blocks_{}".format(i)):
          ### Multihead Attention
          enc = multihead_attention(queries=emb,
                                    keys=emb,
                                    mask=mask,
                                    num_units=config.word_embedding_dim,
                                    num_heads=config.nb_head,
          ...
          ### Feed Forward
          enc = feedforward(enc,
                            num_units=[4 * config.word_embedding_dim, config.word_embedding_dim], scope=scope)
  return enc
...
class SkipThoughtsModel(object):
  ...
  def build_word_embeddings(self):
    ...
    if self.config.attention:
        self.encode_emb = attention(self.config, encode_emb, self.encode_mask, scope='encode')
        self.decode_pre_emb = attention(self.config, decode_pre_emb, self.decode_pre_mask, scope='decode_pre')
        self.decode_post_emb = attention(self.config, decode_post_emb, self.decode_post_mask, scope='decode_post')
    else:
  ...
  def build_encoder(self):
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      if self.config.bidirectional_encoder:
        ...
        _fw_cell = tf.nn.rnn_cell.LSTMCell(num_units, initializer=self.uniform_initializer)
        cell_fw = tf.nn.rnn_cell.DropoutWrapper(_fw_cell, self.config.dropout_keep_prob)
        _bw_cell = tf.nn.rnn_cell.LSTMCell(num_units, initializer=self.uniform_initializer)
        cell_bw = tf.nn.rnn_cell.DropoutWrapper(_bw_cell, self.config.dropout_keep_prob)

        out, states = tf.nn.bidirectional_dynamic_rnn(
            cell_fw=cell_fw,
            cell_bw=cell_bw,
        ...
        encoder_final_state_c = tf.concat(
            (states[0].c, states[1].c), 1)

        encoder_final_state_h = tf.concat(
            (states[0].h, states[1].h), 1)
        ...
        thought_vectors = encoder_final_state

      else:
        _cell = tf.nn.rnn_cell.LSTMCell(self.config.encoder_dim, initializer=self.uniform_initializer)
        cell = tf.nn.rnn_cell.DropoutWrapper(_cell, self.config.dropout_keep_prob)
