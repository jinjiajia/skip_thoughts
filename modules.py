...
def positional_encoding(inputs,
    ...
    # N, T = config.batch_size,config.vocab_size
...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # if mask is not None:
        #     ###mask=length
        #     length_mask = tf.cast(tf.tile(tf.expand_dims(mask, 2), [1, 1, num_units]), tf.float32)
        #     # print("length_mask", length_mask)
        #     Q = tf.where(tf.equal(length_mask, 0), length_mask, Q)
        #     K = tf.where(tf.equal(length_mask, 0), length_mask, K)
        #     V = tf.where(tf.equal(length_mask, 0), length_mask, V)
