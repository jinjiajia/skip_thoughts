...
from skip_thoughts_model_bilstm import SkipThoughtsModel

FLAGS = tf.flags.FLAGS

tf.flags.DEFINE_string("input_file_pattern", './data/output1/train-?????-of-00100',
                       "File pattern of sharded TFRecord files containing "
                       "tf.Example protos.")
tf.flags.DEFINE_string("train_dir", './model/attention',
...
def main(unused_argv):
  ...
  g = tf.Graph()
  with g.as_default():
    model = SkipThoughtsModel(model_config, mode="train")
    model.build()
  ...
  #     print(sess.run([model.encode_ids]))
